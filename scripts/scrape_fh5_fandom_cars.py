#!/usr/bin/env python3
"""Scrape Forza Horizon 5 car data from forza.fandom.com via MediaWiki API.

Writes data/fh5_fandom_cars.json and regenerates supabase/seed/fh5cars.json.
Reuses fh6_fandom_normalize for make/edition resolution.
"""
...
def parse_fh5_car_stats(wt: str) -> dict[str, str]:
...
def main() -> None:
    ...
    index = api_get(
        {"action": "parse", "page": "Forza_Horizon_5/Cars", "prop": "wikitext|text"}
    )
    wt = index["parse"]["wikitext"]["*"]
    html = index["parse"]["text"]["*"]

    templates = re.findall(r"\{\{CarListStatsFH5\|([^}]+)\}\}", wt)
        ...
        row.update(
            {
                ...
                "gears": box.get("gears", ""),
                "in_fh5": "fh5=y" in wt_page or "{{CarStats|fh5" in wt_page.lower(),
    ...
    out_path = Path(__file__).resolve().parents[1] / "data" / "fh5_fandom_cars.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(
            {
                "source": "https://forza.fandom.com/wiki/Forza_Horizon_5/Cars",
    ...
    subprocess.run(
        ["node", str(Path(__file__).resolve().parent / "build-fh5-app-catalog.mjs"), str(out_path)],
        check=True,
