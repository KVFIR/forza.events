...
4. **Discord notifications:** after `017` db push, `npm run deploy:functions` (includes `process-notifications`), set prod `NOTIFICATION_CRON_SECRET`, schedule `scripts/invoke-process-notifications.sh` every minute — outbox rows alone do not deliver DMs
