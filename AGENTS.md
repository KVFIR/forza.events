...
- **Post-start host:** Event Detail shows separate **Submit results** + **Cancel event** buttons; cancel syncs Discord embed via `syncPublishedEmbed`.
  ...
  - **Publish embed:** `publish-event` posts a Discord message embed + **Open event** button (`custom_id` `open_event:{event_id}` via `buildEventEmbed` in `supabase/functions/_shared/events.ts`). Button click → `interactions-endpoint` returns `LAUNCH_ACTIVITY` + stores `launch_intents` fallback. **`bootstrapActivityLaunch()`** in `main.tsx` (before React) rewrites history to `/event/{id}` when `custom_id` is in the iframe URL; otherwise **`pendingEmbedSdkProbe`** holds browse until `sdk.ready()` reads `sdk.customId`, then **`applyEmbedLaunchRoute()`** runs before OAuth. App Launcher → probe finishes with no id → browse loads. No redirect on `guildId` alone; optional **`launch-intent`** only in background on `/`.
