...
- **Implications for UI/UX** (design and copy should assume Activity, not a generic website):
  ...
  - **Publish target** (server + channel) is Discord-native: `list-guilds` = user guilds ∩ servers where the **bot is installed**; `list-channels` = text channels only. Empty server list → **Add to server** CTA (`buildBotInstallUrl` / `openBotInstallUrl` in `src/lib/discordInstall.ts`) — OAuth `scope=bot` with `response_type=code` and redirect `{APP_ORIGIN}/bot-installed` (Discord OAuth2 → Redirects). User-install in App Launcher does **not** add a publish target.
