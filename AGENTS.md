...
- **Post-start host:** Event Detail shows separate **Submit results** + **Cancel event** buttons; cancel syncs Discord embed via `syncPublishedEmbed`.
  ...
  - **Publish embed:** `publish-event` posts a Discord message embed + **Open event** button (`custom_id` `open_event:{event_id}` via `buildEventEmbed` in `supabase/functions/_shared/events.ts`). Button click → `interactions-endpoint` returns `LAUNCH_ACTIVITY` + stores `launch_intents` fallback. **`bootstrapActivityLaunch()`** may rewrite history to `/event/{id}` when `custom_id` is in the iframe URL. **`launchResolutionPending`** blocks `browse-events` until OAuth finishes and **`resolveActivityLaunchAfterAuth()`** runs (`sdk.customId` → `launch-intent` → else unblock browse). App Launcher: brief wait for auth, then browse only. Embed: single-event route, browse feed stays deferred.
