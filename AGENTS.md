...
- **Implications for UI/UX** (design and copy should assume Activity, not a generic website):
  ...
  - **Publish target** (server + channel) is Discord-native: `list-guilds` = user guilds with **Manage Server** (or Administrator) ∩ servers where the **bot is installed** (empty manageable list falls back to all user guilds with bot, but publish still requires Manage Server); `list-channels` = text channels where the bot has **View Channel**, **Send Messages**, **Embed Links**, and **Create Invite** (last needed for `settings.invite_url` on publish); client trusts `list-channels` (in-flight dedupe + 30s cache in `apiDedup.ts`; **Refresh** bypasses cache). **Draft save** with `guild_id` set needs guild **membership** only (`resolveGuildNameForUser` on `save-event`); **publish** and channel pick require **Manage Server** / Administrator (`requireManageGuildAccess` on `list-channels`, `validate-channel`, `publish-event`). Empty server list → **Add to server** CTA (`buildBotInstallUrl` / `openBotInstallUrl` in `src/lib/discordInstall.ts`) — callback-less bot OAuth (`scope=bot`); Activity uses `openExternalLink` → browser, then **Refresh list**. **Bot → Requires OAuth2 Code Grant must be OFF** in the Developer Portal. User-install in App Launcher does **not** add a publish target.
...
6. **Client analytics:** after `021`–`024` db push, `npm run deploy:functions` (includes `track-event`, `analytics-dashboard`, `prune-client-analytics`), `npm run sync:secrets` (`ANALYTICS_TRACK_SECRET` + `ANALYTICS_DASHBOARD_SECRET`), Railway build env **`ANALYTICS_TRACK_SECRET`**, enable `.github/workflows/prune-client-analytics.yml`; after `026` db push, redeploy `analytics-dashboard` + `track-event` (notification aggregates + allowlist)
7. `npm run deploy:functions` or deploy `browse-events` + `host-drafts` with **`--no-verify-jwt`**
8. **`list-guilds` before Profile DM-reachability UI** when shipping `{dm_reachability,fresh}` body support — old Edge ignores the flag and member-only shared servers look unreachable
9. `npx wrangler deploy` when Cloudflare Worker changes (`cloudflare/supabaseProxy.js`, `staticOgHandler.js`, `eventOgHandler.js`, `robotsHandler.js`, `sitemapHandler.js`)
10. Ship frontend (Railway) after any `api.ts` / proxy fetch changes
11. Hard refresh in Discord Activity
12. Run P0 checks in [`docs/E2E.md`](docs/E2E.md) when changing auth, browse, publish, embed sync, add-group, change-group-leader, notifications, or link previews / Worker OG
...
- **Create step tabs:** forward `StepIndicator` navigation runs `tryContinue` (same validation as Continue); backward tabs skip validation.
- **Mobile WebKit datetime-local:** intrinsic min-width overflows narrow viewports — needs `::-webkit-datetime-edit` `min-width: 0` in `index.css` plus `Input` overflow wrapper.
