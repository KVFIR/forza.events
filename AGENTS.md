...
- Migration `017`: changing `leave_event_participant` return type (boolean → jsonb) requires `DROP FUNCTION` + re-grant — `CREATE OR REPLACE` alone fails with `42P13` on existing installs.
- Partial deploy order: **`018` before frontend** (add-group without waitlist; old RPC raises `WAITLIST_EMPTY`); **`020` before frontend** (change-group-leader; RPC missing → Edge runtime failure).
- Discord notifications prod path: `017`+`019` db push + deploy `process-notifications` + `NOTIFICATION_CRON_SECRET` on Supabase Edge and cron runner (GitHub Actions `.github/workflows/process-notifications.yml` every 5 min, or VPS `scripts/invoke-process-notifications.sh`); `NOTIFICATION_CRON_SECRET` on Railway is not needed — mutations also enqueue + `deferNotificationDelivery`.
...
- `scheduleChanged` compares `starts_at` at minute precision (`notificationDiff.ts` + mirrored `publishedEventNotifyDiff.ts`) — sub-minute DB drift must not trigger Save & notify or schedule DMs.
- Reschedule `event_updated` bypasses profile bell (`bypassesDmOptOut` when `scheduleChanged === '1'`); tracks/cars-only `event_updated` still respects `dm_notifications_enabled`.
...
- Change-group-leader capacity: demoting a `self_join` convoy leader keeps the group full; removing a `host_assigned` leader row frees one seat — UI hides waitlist/guild picks and disables guild search when full.
