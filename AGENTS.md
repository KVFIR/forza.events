...
- **Discord DM notifications:** `users.dm_notifications_enabled` (default **on**) + `notification_locale` (`en`/`ru`). Profile bell pill under EN/RU; waitlist promotion DMs always sent. Outbox `notification_outbox`; delivery via `process-notifications` Edge + `deferNotificationDelivery` after mutations. **Claim:** `claim_notification_outbox_batch` (`019`) — `FOR UPDATE SKIP LOCKED` + `processing` status (parallel cron safe). **Prod:** `NOTIFICATION_CRON_SECRET` on Supabase Edge secrets; **GitHub Actions** `.github/workflows/process-notifications.yml` every 5 min (GH schedule minimum) with matching repo secrets — or VPS cron every minute via `scripts/invoke-process-notifications.sh`. Local dev: `ALLOW_INSECURE_NOTIFICATION_CRON=true` without secret. Kinds: cancel (excludes host), convoy leader changed (group racers; draft group-1 on save + `change-group-leader`), convoy leader assigned (new leader on `change-group-leader`; transactional), waitlist seat / new group (+ leader variant), event_updated (date/time + tracks + car rules, active only), 2h reminders (racers + host; 110–130 min scan; dedupe includes `starts_at`; pending/processing 2h rows skipped on schedule change via `save-event`), host group full / lobby full. `change-group-leader` noop (same leader) skips DMs. DM button is link style to `APP_ORIGIN/event/{id}`. Migrations `017` + `019`.
