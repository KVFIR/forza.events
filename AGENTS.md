...
- **Implications for UI/UX** (design and copy should assume Activity, not a generic website):
  ...
  - **Publish target** (server + channel) is Discord-native: `list-guilds` = user guilds ∩ servers where the **bot is installed**; `list-channels` = text channels only. Empty server list → **Add to server** CTA (`buildBotInstallUrl` / `openBotInstallUrl` in `src/lib/discordInstall.ts`) — callback-less bot OAuth (`scope=bot`); Activity uses `openExternalLink` → browser, then **Refresh list**. **Bot → Requires OAuth2 Code Grant must be OFF** in the Developer Portal. User-install in App Launcher does **not** add a publish target.
