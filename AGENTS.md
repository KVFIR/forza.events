...
- **Auth in Activity:** `initDiscordActivity()` → SDK `authorize` (scopes `identify`, `guilds`, `rpc.activities.write`) → `token-exchange` → `authenticate`. Guild context: `sdk.guildId` pre-fills create-event **target server** when the Activity was launched on a server.
- **Rich Presence:** `DiscordRichPresenceSync` + `setActivity()` — English-only copy; `VITE_APP_ORIGIN` for image URLs; host/joined/viewing + event status in `state`, lobby size in `party` only. Overrides on Event Detail, Results, Create (title). `resetRichPresenceSession()` on Activity re-auth. See `src/lib/discordRichPresence.ts`.
