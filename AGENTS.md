...
8. `npx wrangler deploy` when Cloudflare Worker changes (`cloudflare/supabaseProxy.js`, `staticOgHandler.js`, `eventOgHandler.js`, `robotsHandler.js`, `sitemapHandler.js`)
9. Ship frontend (Railway) after any `api.ts` / proxy fetch changes
10. Hard refresh in Discord Activity
11. Run P0 checks in [`docs/E2E.md`](docs/E2E.md) when changing auth, browse, publish, embed sync, add-group, change-group-leader, notifications, or link previews / Worker OG
