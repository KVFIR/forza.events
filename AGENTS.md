...
- **Browser sign-out:** standalone browser (`isStandaloneBrowser()`) shows **Sign out** in `AuthStatusIndicator` (`signOutBrowser` → `clearDiscordSession` + guest reset) so users can switch Discord accounts; Activity auth path unchanged.
...
- Deploy **`upload-cover`** with other functions (`npm run deploy:functions`). Apply migrations with `supabase db push` after linking the project (`001`–`010`; see [`supabase/README.md`](supabase/README.md)).
