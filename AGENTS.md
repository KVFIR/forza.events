...
9. Run P0 checks in [`docs/E2E.md`](docs/E2E.md) when changing auth, browse, publish, embed sync, add-group, or notifications
...
- [`supabase/README.md`](supabase/README.md) — migrations `001`–`018`, Edge Functions
- [`scripts/deploy-edge-functions.sh`](scripts/deploy-edge-functions.sh) — canonical function list (17; includes `add-group`, `process-notifications`)
...
- Discord notifications prod path: `017` db push + deploy `process-notifications` + `NOTIFICATION_CRON_SECRET` + minute cron (`scripts/invoke-process-notifications.sh`); mutations enqueue only.
