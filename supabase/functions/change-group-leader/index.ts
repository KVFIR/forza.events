...
import {fetchDiscordUserById, isUserMemberOfGuild} from '../_shared/discord.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const groupIndex = Number(body.group_index);
    ...
    if (!Number.isInteger(groupIndex) || groupIndex < 1 || groupIndex > 5) {
    ...
    const {data: event} = await supabase
      .from('events')
      .select(
        'host_discord_id, guild_id, status, starts_at, group_count, lobby_leader_discord_id, lobby_leader_is_host, lobby_leader_gamertag, title, timezone_hint, max_players',
      )
      .eq('id', eventId)
      .single();
    ...
    if (groupIndex > (event.group_count ?? 1)) {
      return appErrorResponse(req, 400, API_ERROR_CODES.BAD_REQUEST);
    }
    ...
    const currentLeaderId = resolveActiveGroupLeaderId(roster ?? [], groupIndex, hostProjection);
    if (currentLeaderId === leaderId) {
      return jsonResponse({ok: true, group_index: groupIndex, unchanged: true}, 200, req);
    }

    if (!canPickAsNewGroupLeader(roster ?? [], leaderId, hostProjection)) {
    ...
    if (event.guild_id) {
      try {
        ...
        console.error(
          JSON.stringify({msg: 'change-group-leader guild check failed', detail: String(e)}),
        );
    ...
    const {data: changedGroup, error: rpcErr} = await supabase.rpc('change_event_group_leader', {
      p_event_id: eventId,
    ...
    await enqueueConvoyLeaderChanged(
      ...
      eventRow,
      rosterAfter ?? [],
      Number(changedGroup ?? groupIndex),
      tag.gamertag,
      body.leader_username,
    );
    ...
    if (!embedSync.ok) {
      console.error(
        JSON.stringify({
          msg: 'Changed group leader but Discord embed sync failed',
          eventId,
    ...
    return jsonResponse(
      {ok: true, group_index: changedGroup, embed_synced: embedSync.ok},
      200,
      req,
    );
  } catch (e) {
