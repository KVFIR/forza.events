-- Error brief for local analytics dashboard: distinct timestamps, HTTP status,
-- request host, and Activity-only recent errors.
...
as $$
  ...
  select jsonb_build_object(
    ...
    'top_errors', coalesce((
      ...
      from (
        select
          ...
          count(distinct created_at)::bigint as distinct_ts
        from base
        where event_name = 'api_error'
        group by 1, 2, 3
        order by count desc
    ...
    'errors_by_host', coalesce((
      select jsonb_object_agg(host, cnt)
      from (
        select coalesce(nullif(meta->>'host', ''), 'unknown') as host, count(*)::bigint as cnt
        from base
        where event_name = 'api_error'
        group by 1
      ) h
    ...
    'activity_recent_errors', coalesce((
      select jsonb_agg(row_to_json(t) order by t.at desc)
      from (
        select
          created_at as at,
