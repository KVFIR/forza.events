...
describe('draft validation parity', () => {
  const cases = [
    {
      name: 'title required',
      client: {title: '', type: 'road', startsAtLocal: '2030-01-01T12:00'},
      server: {title: '', type: 'road', starts_at: '2030-01-01T00:00:00.000Z', guild_id: 'g1'},
      code: VALIDATION_CODES.TITLE_REQUIRED,
    },
    {
      name: 'type required',
      client: {title: 'Race', type: 'invalid', startsAtLocal: '2030-01-01T12:00'},
    ...
    },
    {
      ...
      client: {title: 'Race', type: 'road', startsAtLocal: ''},
      server: {title: 'Race', type: 'road', guild_id: 'g1'},
      code: VALIDATION_CODES.STARTS_AT_REQUIRED,
    },
    {
      name: 'draft without guild is valid',
      client: {title: 'Race', type: 'road', startsAtLocal: '2030-01-01T12:00'},
      server: {title: 'Race', type: 'road', starts_at: '2030-01-01T00:00:00.000Z'},
      code: null,
    },
    {
      name: 'valid draft with guild',
      client: {title: 'Race', type: 'road', startsAtLocal: '2030-01-01T12:00'},
...
describe('publish validation parity', () => {
  ...
  it('guild required', () => {
    const code = VALIDATION_CODES.GUILD_REQUIRED;
    expect(validatePublishForm({...base, guildId: null})).toBe(code);
    expect(
  ...
  it('allows open build without a PI cap', () => {
    expect(validatePublishForm({...base, maxPi: null})).toBeNull();
    expect(
      validatePublishReady({
        title: base.title,
        type: base.type,
